class SalesforceData
  ...
  def query_in(query_clause, values, sub_clauses = [])
    results = []
    values.to_a.each_slice(300) do |s|
      values_str = s.inject { |id_str, id| id_str + "','#{id}"}
      values_str = "('#{values_str}')"

      results.concat query(query_clause.gsub('__IN_VALUES__', values_str), sub_clauses)
    end
  ...
  def query(query_clause, sub_clauses = [])
    ...
    raise response.Fault.faultstring if response.Fault
      ...
      @logger.info "Returning 0 records"
      []
      else
      ...
      sub_clauses.each do |c|
        records.each do |r|
          if r[c].nil?
            r[c] = []
          else
            r[c] = r[c].records if r[c].records.is_a? Array
            r[c] = [ r[c].records ] unless r[c].is_a? Array
          end
        end
      end
